...
def read_root():
    """Serve the GUI homepage with no-cache headers"""
...
def convert_rgb_to_yuv(r: int, g: int, b: int):
    ...
    return {"r": r, "g": g, "b": b, "y": y, "u": u, "v": v}

@app.get("/yuv-to-rgb")
def convert_yuv_to_rgb(y: int, u: int, v: int):
    r, g, b = ColorTranslator.yuv_to_rgb(y, u, v)
    return {"y": y, "u": u, "v": v, "r": r, "g": g, "b": b}
...
@app.post("/resize")
def resize(file: UploadFile = File(...), width: int = Query(...), height: int = Query(...), isVideo: bool = Query(...)):
    """
    Exercise 3: Resizes an image or video to specific dimensions.
    """
    input_path = save_upload(file)
