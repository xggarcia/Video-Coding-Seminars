# P2 - API & Dockerization

## Project Overview
This project implements a multimedia processing API using **FastAPI** and **Docker**. It integrates algorithms from Practice 2 (Color spaces, RLE, DCT, DWT) and orchestrates a microservice architecture where heavy video processing is delegated to a separate **FFMPEG container**.
...
1.  **Main API (`practice2-api`)**:
    * Built with **FastAPI**.
    * Handles image processing logic (OpenCV, PIL, NumPy).
    * Contains the logic adapted from Practice 2.
    * Exposes endpoints for user interaction.

2.  **FFMPEG Service (`practice2-ffmpeg-service`)**:
...
Important: You must move into the specific subfolder for Practice 2 before running Docker.
